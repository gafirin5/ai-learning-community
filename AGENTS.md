...
| 2026-08-30 | Desain / Impeccable | Redesign sistem visual **"Rapor & Register"** (seed 94273073, code-led, diminta user): PRODUCT.md + dunia kertas HVS/tinta pulpen/salinan karbon (indigo #4f46e5 terkunci), Archivo + Spline Sans Mono, header/landing/dashboard/login/register/courses/leaderboard/paths dibangun ulang, token RGB+alpha-value (fix bug laten modifier opasitas), ikon SVG ganti emoji, hook `use-leaderboard` bersama | Selesai | lint 0 error baru, tsc 0 non-test, build hijau 22/22; finish-review Impeccable: **ship** (7/7 fix resolved); DESIGN.md + sidecar terekam; **#11 merged (`75b403e`) + LIVE di VPS**; Gelombang 2: forum/profil/proyek/detail dibangun-ulang struktural + `formatTanggal` — **#12 merged (`e45127d`), #13 (exception detector) (`2d8f5ad`), deploy ulang VPS smoke 7 route 200** |
| 2026-09-10 | Audit & tidy-up + gelombang fitur (langsung, non-lane) | 1) Tidy: hapus `benchmark.mjs` (scratch usang), fix bug path `scripts/deploy-check.js` (`__dirname`→root). 2) Update `FEATURES.md` total — sinkronkan dokumen rencana usang dengan status kode nyata (audit ketemu gap: AI Tutor Production/OpenRouter di `CONTRACTS.md` diklaim "Published" tapi filenya tak pernah ada; Testing Infra cuma 2 file test). 3) Fitur baru: `SessionVideo.tsx` mentor diupgrade dari stub statis → pratinjau kamera/mic nyata (`getUserMedia`) + copy Indonesia; halaman baru **Code Playground** (`/playground`, sandbox JS via iframe sandbox, tanpa dependency baru) + link dari dashboard; unit test baru untuk `sm2.ts`/`quota.ts`/`learning-path.ts` (37 test, sebelumnya 0). 4) Perbaikan tak terduga yang ditemukan saat verifikasi: `npm run lint` ternyata sudah merah sejak commit `d9f5922`/`90dbcd1` (unused vars di `useChat.ts` + `any`/unused di test mocks) — dibersihkan; `npx tsc --noEmit` berjalan tanpa tipe `vitest/globals` di `tsconfig.json` (82 error palsu) — ditambahkan `types: ["vitest/globals"]`, sisa error nyata di `fixtures/course.ts`/`mocks/store.ts` (drift skema `Course`/`Lesson`/`Question`/`StoreState` vs mock lama) diperbaiki; test `date.test.ts` dibetulkan (bandingkan UTC vs UTC, bukan UTC vs lokal) | Selesai | `npm run lint` 0 error (sebelumnya merah, bukan cuma dari kerjaan sesi ini), `npx tsc --noEmit` 0 error (sebelumnya 82 error semu + 19 nyata), `npx vitest run --pool=threads` 47/47 pass (sebelumnya 9/10, 0 test untuk sm2/quota/learning-path), `npm run build` hijau 23/23 routes (`/playground` baru). **Belum dikerjakan (didokumentasikan, bukan diabaikan):** AI Tutor LLM sungguhan (butuh `OPENROUTER_API_KEY`, di luar kapasitas sesi ini), koneksi video peer-to-peer mentor sungguhan (butuh provider WebRTC/Daily/Zoom berbayar) |
| 2026-09-12 | Lane C (Catatan) + Lab (Deck Kartu Hafalan), branch `feat/notes-and-flashcard-decks` | Dua fitur baru diminta user langsung: 1) **Catatan Pribadi per Pelajaran** — panel autosave privat di halaman pelajaran, tabel `lesson_notes` RLS owner-only, tanpa RPC. 2) **Deck Kartu Hafalan buatan sendiri + sharing/template** — perluasan fitur Kartu Hafalan (SRS) yang sudah ada: user bisa buat deck sendiri, bagikan publik ke komunitas, dan menduplikat ("pakai sebagai template") deck orang lain. Tabel baru `flashcard_decks`/`flashcard_cards` + RPC `save_flashcard_deck` (ganti kartu atomik)/`clone_flashcard_deck`/`list_flashcard_decks`; id `flashcard_cards` mulai dari 100000 agar tidak bertabrakan dengan id kartu bawaan statis (1-33) di `flashcard_progress`. Kartu bawaan & halaman `/flashcards` lama **tidak diubah** (penambahan murni, zero regression risk) — hanya ditambah 1 link ke galeri deck baru. | Sebagian | `npm run lint` 0 error, `npx tsc --noEmit` 0 error, `npm run build` hijau 28/28 routes (3 route baru: `/flashcards/decks`, `/flashcards/decks/new`, `/flashcards/decks/[id]`). **Belum di-commit ke `main`** — kerja ada di branch `feat/notes-and-flashcard-decks`, belum di-PR. **2 migration baru (`20260912000001_lesson_notes.sql`, `20260912000002_flashcard_decks.sql`) belum di-apply** ke Supabase live (tidak ada akses kredensial DB di sesi ini) — terapkan via SQL Editor atau `supabase db push` sebelum merge; tanpa migration, kedua fitur tampil sebagai gate "Supabase belum dikonfigurasi"/tersembunyi, tidak crash. Vitest tidak dijalankan ulang (tidak ada test baru ditambahkan untuk 2 fitur ini). |

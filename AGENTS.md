...
| 2026-08-29 | Lab / eksperimental (branch test1) | Learning Lab: flag `/labs` + Jalur Belajar (mastery gate 80% + enrollment `path_enrollments` + bonus +50 via `claim_path_bonus` + badge pioneer/graduate) + Kartu Hafalan SRS SM-2 (`flashcard_progress`, 15 kartu seed) + widget dashboard | Sebagian | lint (0 error baru), tsc 0 non-test, build hijau 19/19 routes di test1 (`f9aad59`..`e28b7a2`); 2 migration (20260902000001/2) **belum applied** — password pooler lama invalid (28P01), menunggu DB password baru / apply via SQL Editor; NAV site-header +1 "Jalur" (mohon review Lane A) |
